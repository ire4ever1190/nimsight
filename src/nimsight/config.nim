...
let validBinary = validator[Path](path => path.fileExists(), path => fmt"'{$path}' does not point to a file")

let validator = NimConfig.validator()(nimBinary = @[validBinary], nimbleBinary = @[validBinary])
